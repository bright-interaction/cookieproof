...
// ---------------------------------------------------------------------------
{
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS custom_domains (
      ...
      cname_target TEXT NOT NULL DEFAULT 'consent.example.com',
...
// ---------------------------------------------------------------------------
{
  ...
  if (!hasGoldReject) {
    const seedConfig = {
      ...
      proofEndpoint: "https://consent.example.com/api/proof",
...
function getBillingEmailHtml(
  ...
  const reactivateLink = data.reactivateUrl || "https://consent.example.com/configurator/#billing";
...
<body style="font-family:system-ui,-apple-system,sans-serif;background:#f8fafc;padding:40px 20px;">
  <div style="max-width:480px;margin:0 auto;background:#fff;border-radius:12px;padding:32px;border:1px solid #e2e8f0;">
    ...
    if (req.method === "POST" && path === "/api/agency/custom-domains") {
      ...
      const now = Date.now();
      db.prepare(
        "INSERT INTO custom_domains (id, agency_id, domain, cname_target, verified, created_at) VALUES (?, ?, ?, 'consent.example.com', 0, ?)"
      ...
      return cors(json({
        id,
        domain,
        cname_target: "consent.example.com",
        verified: false,
        created_at: now,
      }), origin);
    }
